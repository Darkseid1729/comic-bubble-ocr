...
\author{
    \textbf{Aditya Gautam} (Roll No: cs23b2037) \\
    \textbf{Vansh Singh} (Roll No: cs23i1028) \\
    \textbf{Riyansh Singh bhadouriya} (Roll No: cs23b2038) \\
    \vspace{0.5cm} \\
...
The following table outlines the specific roles and technical contributions of each team member towards the development of the Bubble OCR system.

\begin{table}[H]
    \centering
    \begin{tabular}{@{}llp{8cm}@{}}
    \toprule
    \textbf{Team Member} & \textbf{Role} & \textbf{Key Contributions} \\ \midrule
    Aditya Gautam & DIP Lead & Developed the core preprocessing pipeline, including Bilateral filtering, CLAHE implementation, and Canny edge detection. Optimized Tesseract fallback modes for vertical text. \\
    Vansh Singh & CV Specialist & Implemented the bubble segmentation logic using Watershed algorithms and contour analysis. Designed the geometric filters (Solidity, Circularity) for bubble classification. \\
    Riyansh Singh bhadouriya & Full-Stack Lead & Designed and developed the React-based Studio UI. Built the FastAPI backend integration and managed the deployment process for the standalone executable. \\ \bottomrule
    \end{tabular}
...
The project is fully open-source and hosted on GitHub. The repository contains the full source code for both the Python-based DIP pipeline and the React-based frontend Studio.

\begin{itemize}
    \item \textbf{Source Code:} \href{https://github.com/Darkseid1729/comic-bubble-ocr.git}{github.com/Darkseid1729/comic-bubble-ocr}
    \item \textbf{Standalone Application:} To ensure ease of use for non-technical users, we have packaged the entire system (Backend server + Frontend UI) into a single, standalone **Windows Executable (.exe)** using PyInstaller. This executable bundles all dependencies (OpenCV, Tesseract, and FastAPI) into a portable format that can be run on any Windows system with a single click.
\end{itemize}
